...
/**
 * Copy+Paste of Default to read brightness instead of ink from genome.
 * 
 * @author Nick Beato
 */
final class GreyOutputLayer implements OutputLayer {

	private static DefaultNeuron find(Collection<Neuron> neurons, long id){
		for(Neuron neuron: neurons){
			if(neuron.getNode().getType().equals(NeuronType.OUTPUT) && neuron.getNode().getInnovationId() == id){
	 ...
	 */
	public GreyOutputLayer(Collection<Neuron> neurons){
		output = find(neurons, "ink");
		if (output == null) output = find(neurons, 4);
		values = new double[3];
