...
public class NeatIdMap {
	...
	private Map<List<Long>, Long> connectionToConnectionId = new HashMap<List<Long>, Long>();

	private Map<Long, Long> connectionToNeuronId = new HashMap<Long, Long>();
	 ...
	 */
	protected List<Long> buildList( Long srcNeuronId, Long destNeuronId ) {
	...
	public Long findConnectionId( Long srcNeuronId, Long destNeuronId ) {
		return connectionToConnectionId.get( buildList( srcNeuronId, destNeuronId ) );
	}
...
//		aLogger.log( pri, "neurons->connection id map size == " + connectionToConnectionId.size() );
	...
	public String toXml() {
		...
		for (Long connId : connectionToNeuronId.keySet()) {
			Long neuronId = connectionToNeuronId.get( connId );
		...
		for (List<Long> key : connectionToConnectionId.keySet()) {
			Long srcNeuronId = key.get( 0 );
			Long destNeuronId = key.get( 1 );
			Long connId = connectionToConnectionId.get( key );
