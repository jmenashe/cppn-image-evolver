...
private static final String NEURON_XML_LABEL_TAG = "label";
...
public static NeuronAllele neuronFromXml( Node node ) throws IllegalArgumentException {
	...
	String label = "";
	Node aLabel = atts.getNamedItem( XmlPersistableAllele.NEURON_XML_LABEL_TAG );
	if (aLabel != null)
		label = aLabel.getNodeValue();

	ActivationFunction activationType = ActivationFunction.SIGMOID;
	Node actNode = atts.getNamedItem( XmlPersistableAllele.NEURON_XML_ACTIVATION_TYPE_TAG );
	if ( actNode != null ) {
		str = actNode.getNodeValue();
		activationType = ActivationFunction.get( str );
		if ( activationType == null )
			throw new IllegalArgumentException( "invalid activation function type: " + str );
	}

	return new NeuronAllele( new NeuronGene( type, id, activationType, label ) );
}
...
public String toXml() {
	...
	if ( allele instanceof NeuronAllele ) {
		...
		if (!nAllele.getLabel().equals(""))
			result.append( NEURON_XML_LABEL_TAG ).append( "=\"" ).append(nAllele.getLabel() );
		result.append( "\"/>\n" );
	}
