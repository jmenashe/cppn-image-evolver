	...
	if ( GeneticEvent.GENOTYPE_EVOLVED_EVENT.equals( event.getEventName() ) ) {
		...
		for (Chromosome c : genotype.getChromosomes()) {
			if ( c.getFitnessValue() == maxFitnessValue ) {
				logger.info( "max: id=" + c.getId() + " score=" + fitness + " nodes=" + c.countNodes() + " connections=" + c.countLinks() );
				++maxFitnessCount;
			}
		}
// TODO		if ( maxFitnessCount > 0 )
			logger.info( "# chromosomes with max fitness: " + maxFitnessCount );
		logger.info( "champ: id=" + fittest.getId() + " score=" + fitness + " nodes=" + fittest.countNodes() + " connections=" + fittest.countLinks() );
	}
}
