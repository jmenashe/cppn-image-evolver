...
public class EventManager
{
     ...
     */
    private Map<String, List<GeneticEventListener>> m_listeners = new HashMap<String, List<GeneticEventListener>>();
    ...
    {
        List<GeneticEventListener> eventListeners = m_listeners.get( a_eventName );

        if( eventListeners == null )
        {
            eventListeners = new LinkedList<GeneticEventListener>();
    ...
    {
        List<GeneticEventListener> eventListeners = m_listeners.get( a_eventToFire.getEventName() );

        if ( eventListeners != null ) {
            // Iterate over the listeners and notify each one of the event.
            // ------------------------------------------------------------
            for (GeneticEventListener e : eventListeners) {
                    e.geneticEventFired( a_eventToFire );
            }
        }
    }
