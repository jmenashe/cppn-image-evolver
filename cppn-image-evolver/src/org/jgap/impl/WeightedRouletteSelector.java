...
public class WeightedRouletteSelector extends NaturalSelector {
	 ...
	 */
	private Map<Chromosome, SlotCounter> m_wheel = new HashMap<Chromosome, SlotCounter>();
	...
	protected synchronized List<Chromosome> select( Configuration a_activeConfiguration, int a_howManyToSelect ) {
		...
		if ( m_wheel.isEmpty() == false ) {
			...
			// -------------------------------------------------------------------
			Set<Map.Entry<Chromosome, SlotCounter>> entries = m_wheel.entrySet();
			...
			int i=0;
			for (Map.Entry<Chromosome, SlotCounter> chromosomeEntry : entries) {
				Chromosome currentChromosome = chromosomeEntry.getKey();
